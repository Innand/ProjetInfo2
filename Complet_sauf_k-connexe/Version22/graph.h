...
class Vertex
{
    ...
    public:
        ...
        Vertex (double value=0, VertexInterface *interface=nullptr) :
            m_value(value), m_interface(interface)  { m_actif=true; m_marque=false; }
...
class Edge
{
    ...
    public:
        ...
        Edge (double weight=0, EdgeInterface *interface=nullptr) :
            m_weight(weight), m_interface(interface)  { m_num_arete=0; m_from=0; m_to=0;}

        Edge (int id_vert1, int id_vert2, double weight=0, EdgeInterface *interface=nullptr) :
            m_weight(weight), m_interface(interface)  { m_num_arete=0; m_from=0; m_to=0;}
...
class GraphInterface
{
    ...
    private :
        ...
        grman::WidgetButton m_option_5;
        grman::WidgetText m_option5_label;

        grman::WidgetButton m_option_6;
        grman::WidgetText m_option6_label;

        grman::WidgetButton m_option_7;
        grman::WidgetText m_option7_label;
...
class Graph
{
    public:
        ...
        int m_ordre, m_nb_aretes;

        std::vector<std::string> m_nom_images;

        std::vector<Sommet> m_vect_sommets;

        std::vector<Edge> m_vect_aretes;

        /// Les constructeurs sont à compléter selon vos besoin...
        /// Ici on ne donne qu'un seul constructeur qui peut utiliser une interface
        Graph (GraphInterface *interface=nullptr) :
            m_interface(interface)  { m_ordre=0; m_nb_aretes=0;}
        ...
        std::string accueil(BITMAP* fond_menu, bool *stop);
        ...
        void recup_donnees_som(std::string nom_fic);
        ...
        void k_connexe(int k, Graph g);

        std::vector<std::vector <int>> combinaisons ( int k);
...
void compo_fort_connexes( std::vector <std::vector <int> > *mes_compo);
Graph graph_reduit(std::vector <std::vector <int> > *mes_compo,Graph g);
